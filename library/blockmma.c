...
int blockmma_f128(int devfd, float *a, float *b, float *c, int m, int n, int k, int tile)
{
    ...
    printf("BLOCKMMA: IN blockmma_f128 %f %f %f \n", a[0],b[0],c[0]);
...
int blockmma_f128_accelerator(int devfd)
{
    ...
    while(1)
    {
        ...
        {
            printf("BLOCKMMA.C: Task ID in blockmma %llu\n", cmd.tid);
            printf("BLOCKMMA.C: Value in cmd %llu %llu %llu\n",cmd.a,cmd.b,cmd.c);
            for(i = 0; i < 128; i++)
                for(j = 0; j < 128; j++)
                    for(k = 0; k < 128; k++){
                        // if ( i == 0 && j == 0 && k ==0 ) {
                        //   printf("a in accelerator: %f ", a[i*128+k]); //delete this
                        //   printf("b in accelerator: %f ",b[k*128+j]); //delete this
                        // }
